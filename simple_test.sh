...
curl -s http://localhost:8100/ | python3 -c "import sys, json; print(json.dumps(json.load(sys.stdin), indent=2))"
echo

echo "2. Testing health endpoint:"
curl -s http://localhost:8100/health | python3 -c "import sys, json; print(json.dumps(json.load(sys.stdin), indent=2))"
echo
...
response=$(curl -s -X POST http://localhost:8100/inference \
